class UserStatsService {
  ...
  updateStats(userId, updates) {
    ...
    localStorage.setItem(`${this.storagePrefix}${userId}`, JSON.stringify(updatedStats));
    return updatedStats;
  }
    ...
    if (!isAlreadyWatched) {
      ...
      if (movie.genre_ids) {
        movie.genre_ids.forEach(genreId => {
          stats.favoriteGenres[genreId] = (stats.favoriteGenres[genreId] || 0) + 1;
        });
      }
  ...
  addMovieRating(userId, movieId, rating) {
    ...
    stats.averageRating = stats.totalRatings > 0 ? stats.ratingSum / stats.totalRatings : 0;
